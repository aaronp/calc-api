...
def asHttpResponse[A](
    serviceResponse: Either[String, ServiceResponse[A]]
): cask.Response[String] = {
  serviceResponse match {
    case Right(ServiceResponse.Ok(value, headers)) =>
      cask.Response(value.toString, 200, headers = headers.toSeq)
    case Right(ServiceResponse.NotFound(headers)) =>
      cask.Response("Not Found", statusCode = 404, headers = headers.toSeq)
